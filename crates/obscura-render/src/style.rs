...
pub fn supports_declaration(name: &str, value: &str) -> bool {
    ...
    let known = matches!(
        ...
        "display"
            ...
            | "box-shadow"
            | "-webkit-box-shadow"
            // Recognised by the CSSOM but not acted on by the painter.
            // A version is identified partly by which property names it
            // knows, and an unimplemented declaration is inert either
            // way: CSS drops declarations it does not understand, so
            // admitting the name changes what is reported, not what is
            // painted. Keeping this in step with _CSS_PROPERTY_NAMES is
            // what stops `CSS.supports` and the CSSOM disagreeing.
            | "additive-symbols"
            | "app-region"
            | "ascent-override"
            | "base-palette"
